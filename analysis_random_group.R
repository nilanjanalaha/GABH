...
pool.t.test <- function(mydata, impute=FALSE)
{
  n <- nrow(mydata)
...
lsl.bh <- adaptiveBH(pval, alpha.fix, silent=TRUE)
name <- lsl.bh$rejected
#if(length(name)==0) print("Error: no tests are significant for BH")

my.lsl.bh <- data.frame(mydata[name,c(1:3,5)], pval= round(pval[name],4),
                    missing = miss.val[name,1], regulation = regulation[name],
                    group_id = group[name],
                    type="LSL-BH")
...
tst.bh <- two.stage(pval, alpha.fix)
name <- tst.bh$rejected
#if(length(name)==0) print("Error: no tests are significant for BH")

my.tst.bh <- data.frame(mydata[name,c(1:3,5)], pval= round(pval[name],4),
                        missing = miss.val[name,1], regulation = regulation[name],
                        group_id = group[name],
                        type="TST-BH")
...
storey.bh <- qvalue(pval, fdr.level=alpha.fix, lambda=0.5)
name <-storey.bh$significant
#if(length(name)==0) print("Error: no tests are significant for BH")

storey.bh <- data.frame(mydata[name,c(1:3,5)], pval= round(pval[name],4),
                        missing = miss.val[name,1], regulation = regulation[name],
                        group_id = group[name],
                        type="Storey-BH")
#---------- Group for control study------------------
unigroup <- rep(1,522)
uni_assign <- list(unigroup)
...
#lsl.bh <- gbh(alpha.fix, pval, unigroup, uni_assign, mydata, "lsl", chr)
